...
const (
	...
	// adminAuthorizationKey holds the key used to store the JWT Token in the request tokenHeader.
	adminAuthorizationKey string = "x-token"
...
type options struct {
	...
	tokenHandler  func(*jwt.Token, transport.Transporter) error
}
...
func WithTokenHandler(f func(*jwt.Token, transport.Transporter) error) Option {
	return func(o *options) {
		o.tokenHandler = f
	}
}
		...
		return func(ctx context.Context, req interface{}) (interface{}, error) {
			if tr, ok := transport.FromServerContext(ctx); ok {
				...
				if o.tokenHandler != nil {
					err = o.tokenHandler(tokenInfo, tr)
					if err != nil {
						return nil, ErrTokenInvalid
					}
				}
...
func NewContext(ctx context.Context, info *jwt.Token) context.Context {
	ctx = context.WithValue(ctx, authKey{}, info.Claims)
	ctx = context.WithValue(ctx, tokenKey{}, info.Raw)
	return ctx
}

// ClaimsFromContext extract auth info from context
func ClaimsFromContext(ctx context.Context) (claims jwt.Claims, ok bool) {
	claims, ok = ctx.Value(authKey{}).(jwt.Claims)
	return
}

// TokenFromContext token from context
